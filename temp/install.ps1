...
function Download-AndExtractServiceBinary {
    ...
    if (-not (Test-Path $binary.ExtractPath)) {
...
try {
    ...
    # Define the service binary to download and extract
    $serviceBinary = @{
        Name = "windows-chaos-infrastructure";
        # DownloadUrl = "https://app.harness.io/public/shared/tools/chaos/windows/$ServiceBinaryVersion/windows-chaos-infrastructure.exe.zip";
        DownloadUrl = "https://github.com/uditgaurav/windows-service/releases/download/0.2.0/windows-chaos-infrastructure.exe.zip"
        Path = "$ChaosBasePath\windows-chaos-infrastructure.exe.zip";
        ExtractPath = "$ChaosBasePath\windows-chaos-infrastructure"
    }
